...
void loop() {
 ...
 /* 2. 鎖定原點 Offset (修正版) */
  /* 2. 鎖定原點 Offset (改為開機相對180度版) */
  if (first_data_received && mpu_ready && !origin_set) {
    // 公式推導：我們希望 translated_target_yaw = wrap360(raw_target_yaw - yaw_offset) == 180.0f
    // 所以反推可得：
    yaw_offset = wrap360(raw_target_yaw - 180.0f); 
    
    // 同步將小車當前的角度初始化為 180.0 度
    // 這樣 translated_target_yaw(180) - current_yaw(180) == 0，小車開機時就不會動
    current_yaw = 0.0f; 
